...
def cb_wrapper(func):
    async def wrapper(client, cb):
        ...
        if cb.from_user.id not in users:
            await cb.answer(
                "ഇനി ഇതിൽ തൊട്ട് കളിക്കുന്നവരെ നിയമപരമായും കായികപരമായും നേരിടും..അത് പി-എം ൽ ഇരുന്ന് കുശു പറയുന്ന വരയ്യാല്ലം ഗ്രൂപ്പിൽ ഇരുന്നു താളത്തിൽ പറയുന്നവർ ആയാല്ലം ശരി 😁",
                cache_time=0,
                show_alert=True,
            )
        else:
